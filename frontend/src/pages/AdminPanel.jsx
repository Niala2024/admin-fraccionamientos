...
function AdminPanel() {
  ...
  // ✅ MODIFICACIÓN 1: Agregar 'quejasPendientes' al estado inicial
  ...
  const cargarDatos = async () => {
    ...
    try {
      ...
      // ✅ MODIFICACIÓN 2: Cargar también las quejas
      const [resCasas, resCalles, resUsers, resQuejas] = await Promise.all([
          ...
          api.get('/api/quejas/', { headers }) // <--- Nuevo endpoint
      ]);
      ...
      // Stats originales + Quejas
      const casasFiltradasCalc = (Array.isArray(dataCasas) ? dataCasas : []).filter(c => !fraccSeleccionado || c.fraccionamiento === fraccSeleccionado);
      ...
      setStats({ 
          ...
          quejasPendientes: qPendientes // <--- Guardamos el dato
      });
  ...
  // (El useEffect de deuda se integró en cargarDatos para optimizar, pero mantenemos la referencia por si acaso)
  useEffect(() => {
      ...
      // Solo actualizamos deuda y morosos aquí para no sobrescribir quejas
      setStats(prev => ({ ...prev, deudaTotal: deuda, casasConDeuda: morosos, totalCasas: casasFiltradas.length }));
  ...
  // ✅ MODIFICACIÓN 3: KpiCard con onClick para hacerlo clickeable
      ...
      <Container maxWidth="xl" sx={{ mt: 5, mb: 5, flexGrow: 1 }}>
        <Paper elevation={3} sx={{ p: 3, mb: 4, borderRadius: 3 }}>
            ...
            <Grid container spacing={2}>
                ...
                {/* ✅ MODIFICACIÓN 4: Botón de Quejas */}
                <Grid size="auto">
                    <Button 
                        variant="contained" 
                        sx={{ bgcolor: '#d32f2f', fontWeight: 'bold' }} 
        ...
        <Grid container spacing={3} sx={{ mb: 5 }}>
          {/* ✅ MODIFICACIÓN 5: Grid de 4 columnas (md: 3) para que quepa el cuadro de quejas */}
          <Grid size={{ xs: 12, md: 3 }}><KpiCard title="Deuda Total" value={`$${stats.deudaTotal.toLocaleString()}`} subtitle="Pendiente" icon={<AttachMoneyIcon />} color="#e53935"/></Grid>
          <Grid size={{ xs: 12, md: 3 }}><KpiCard title="Morosidad" value={stats.casasConDeuda} subtitle="Casas con deuda" icon={<ReportProblemIcon />} color="#fb8c00"/></Grid>
          <Grid size={{ xs: 12, md: 3 }}><KpiCard title="Propiedades" value={stats.totalCasas} subtitle="Registradas" icon={<HomeIcon />} color="#1976d2"/></Grid>
          <Grid size={{ xs: 12, md: 3 }}>
            <KpiCard 
                title="Quejas Pendientes" 
                value={stats.quejasPendientes} 
                subtitle="Clic para atender" 
                icon={<ForumIcon />} 
                color="#d32f2f" 
                onClick={() => navigate('/quejas')}
            />
