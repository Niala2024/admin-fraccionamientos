...
// Componente fuera para evitar re-render
...
function Caseta() {
  ...
  useEffect(() => {
    ...
    cargarDatos();
    cargarBitacora(); // Cargar bitácora inicial

    const dataInterval = setInterval(() => {
        cargarDatos();
        cargarBitacora(); // Refrescar bitácora periódicamente
  ...
  const cargarDatos = async () => {
    try {
      ...
      // ... (Procesamiento de gente adentro igual que antes) ...
  ...
  // ✅ FUNCIÓN CARGAR BITÁCORA (HOY)
  const cargarBitacora = async () => {
      try {
          const res = await api.get('/api/reportes-diarios/'); // El backend ya filtra por HOY por defecto
  ...
  const buscarHistorial = async () => {
      ...
      try {
          const res = await api.get(`/api/reportes-diarios/?fecha=${fechaHistorial}`);
  ...
  // ... (Funciones de Chat: cargarChat, seleccionarVecino, enviarMensaje, handleToggleArchivo se mantienen igual) ...
  ...
  // ... (Funciones de Acceso: handleSalida, handleRegistroManual se mantienen igual) ...
  const handleSalida = async (item) => { if (!confirm(`¿Registrar salida de: ${item.nombre}?`)) return; try { if (item.tipo_registro === 'visita') await api.patch(`/api/visitas/${item.id}/`, { fecha_salida_real: new Date() }); else await api.patch(`/api/accesos-trabajadores/${item.id}/`, { fecha_salida: new Date() }); enqueueSnackbar('Salida registrada', { variant: 'info' }); cargarDatos(); } catch (e) { enqueueSnackbar('Error', { variant: 'error' }); } };
  const handleRegistroManual = async () => { if(!formManual.nombre || !formManual.destino) return enqueueSnackbar('Faltan datos', {variant: 'warning'}); try { await api.post('/api/visitas/', { ...formManual, metodo: 'MANUAL', fecha_llegada_real: new Date() }); enqueueSnackbar('Acceso registrado', { variant: 'success' }); setFormManual({ nombre: '', destino: '', tipo: 'Visita', placas: '' }); cargarDatos(); } catch (e) { enqueueSnackbar('Error', { variant: 'error' }); } };

  // ✅ MODIFICADO: REGISTRAR INCIDENTE Y RECARGAR LISTA
  const handleIncidente = async () => { 
      if(!incidente) return; 
      await api.post('/api/reportes-diarios/', { mensaje: incidente, tipo: 'INCIDENTE' }); 
      setIncidente(''); 
      enqueueSnackbar('Bitácora actualizada', { variant: 'warning' }); 
      cargarBitacora(); // Actualizar visualmente al instante
  };
  ...
  useEffect(() => {
      if (tabActionIndex === 0 && document.getElementById("reader")) {
          ...
          scanner.render((decoded) => { setQrResult({ valido: true, mensaje: "Lectura: " + decoded }); enqueueSnackbar('Código Escaneado', {variant:'success'}); });
    ...
    <Box sx={{ flexGrow: 1, height: '100vh', display: 'flex', flexDirection: 'column', bgcolor: themeColors.bgMain, overflow:'hidden' }}>
      ...
      <Grid container spacing={2} sx={{ flexGrow: 1, p: 2, overflow: 'hidden' }}>
            ...
            <Paper elevation={2} sx={{ height: '100%', bgcolor: themeColors.bgPaper, borderRadius: 3, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
                ...
                <List sx={{ overflowY: 'auto', flexGrow: 1, p: 1 }}>
                    {genteAdentro.length === 0 ? (
                        <Box sx={{ textAlign: 'center', mt: 4, color: themeColors.textSecondary, opacity: 0.7 }}><DirectionsCarIcon sx={{ fontSize: 40, mb: 1 }} /><Typography variant="body2">Fraccionamiento Vacío</Typography></Box>
        ...
        <Grid item xs={12} md={5} lg={6} sx={{ height: '100%' }}>
           <Paper elevation={2} sx={{ height: '100%', bgcolor: themeColors.bgPaper, borderRadius: 3, display:'flex', flexDirection:'column', overflow:'hidden' }}>
             <Tabs value={tabActionIndex} onChange={(e,v)=>setTabActionIndex(v)} centered sx={{ borderBottom: `1px solid ${themeColors.bgLight}`, '& .MuiTab-root': { color: themeColors.textSecondary }, '& .Mui-selected': { color: themeColors.accent + '!important' } }} TabIndicatorProps={{ style: { backgroundColor: themeColors.accent } }}>
             ...
             <Box sx={{ p: 3, flexGrow: 1, overflowY: 'auto', display:'flex', flexDirection:'column' }}>
                {tabActionIndex === 0 && (
                    <Box textAlign="center" flexGrow={1} display="flex" flexDirection="column" justifyContent="center">
                        <Typography variant="h5" color={themeColors.textPrimary} gutterBottom fontWeight="bold">Escanear Pase</Typography>
                        <Paper elevation={4} sx={{ width: '100%', maxWidth: 480, mx: 'auto', bgcolor: '#000', borderRadius: 3, overflow:'hidden', border: `2px solid ${themeColors.accent}` }}><div id="reader" style={{ width: '100%' }}></div></Paper>
                        {qrResult && <Chip label={qrResult.mensaje} color={qrResult.valido?'success':'error'} sx={{mt:3}} />}
                    </Box>
                )}

                {tabActionIndex === 1 && (
                    <Box maxWidth="sm" mx="auto" width="100%">
                        <Box mb={3} textAlign="center"><Typography variant="h6" color={themeColors.accent} fontWeight="bold">Nuevo Ingreso</Typography></Box>
                        <Grid container spacing={2}>
                            <Grid item xs={12}><CustomTextField label="Nombre del Conductor / Visitante" value={formManual.nombre} onChange={e=>setFormManual({...formManual, nombre:e.target.value})} InputProps={{startAdornment: <InputAdornment position="start"><PersonIcon sx={{color:themeColors.textSecondary}}/></InputAdornment>}} /></Grid>
                            <Grid item xs={12}><CustomTextField select label="Tipo de Visita" value={formManual.tipo} onChange={e=>setFormManual({...formManual, tipo:e.target.value})}>
                                <MenuItem value="Visita">Visita Social</MenuItem><MenuItem value="Proveedor">Proveedor / Servicio</MenuItem><MenuItem value="Taxi">Taxi / Uber / Didi</MenuItem><MenuItem value="Paqueteria">Paquetería</MenuItem><MenuItem value="CFE">CFE / Luz</MenuItem><MenuItem value="Emergencia">Emergencia</MenuItem>
                            </CustomTextField></Grid>
                            <Grid item xs={12}><CustomTextField label="Placas" value={formManual.placas} onChange={e=>setFormManual({...formManual, placas:e.target.value})} InputProps={{startAdornment: <InputAdornment position="start"><DirectionsCarIcon sx={{color:themeColors.textSecondary}}/></InputAdornment>}}/></Grid>
                            ...
                            <Grid item xs={12}><Button fullWidth variant="contained" size="large" sx={{bgcolor:themeColors.accent, color:'white', py:1.8, borderRadius: 2}} onClick={handleRegistroManual}>AUTORIZAR INGRESO</Button></Grid>
                        </Grid>
                    </Box>
                )}

                {/* ✅ TAB BITÁCORA CON LISTA Y BOTÓN HISTORIAL */}
                {tabActionIndex === 2 && (
                    <Box maxWidth="md" mx="auto" width="100%" display="flex" flexDirection="column" height="100%">
                        <Box flexShrink={0}>
                            ...
                            <Button fullWidth variant="contained" color="warning" sx={{mt:2, mb:3, py:1.5, borderRadius: 2}} onClick={handleIncidente} startIcon={<BookIcon/>}>REGISTRAR NOVEDAD</Button>
                            <Box display="flex" justifyContent="space-between" alignItems="center" mb={1}>
                                <Typography variant="subtitle2" color={themeColors.textSecondary}>Bitácora del Turno (Hoy)</Typography>
                                <Button size="small" startIcon={<HistoryIcon/>} sx={{color: themeColors.accent}} onClick={()=>setOpenHistorial(true)}>Consultar Historial</Button>
                            </Box>
                        </Box>
                        
                        {/* LISTA DE REGISTROS DEL DÍA */}
                        <Paper sx={{ flexGrow: 1, overflowY: 'auto', bgcolor: themeColors.bgMain, p: 1, borderRadius: 2, border: `1px solid ${themeColors.bgLight}` }}>
                            {listaBitacora.length === 0 ? (
                                <Typography align="center" variant="caption" color="textSecondary" display="block" mt={2}>Sin novedades registradas hoy</Typography>
                                ...
                                <Box key={log.id} sx={{ p: 1.5, mb: 1, bgcolor: themeColors.bgPaper, borderRadius: 1, borderLeft: `4px solid ${themeColors.warning}` }}>
                                    <Box display="flex" justifyContent="space-between">
                                        <Typography variant="body2" color={themeColors.textPrimary}>{log.mensaje}</Typography>
                                        <Typography variant="caption" color={themeColors.textSecondary}>{new Date(log.fecha).toLocaleTimeString([], {hour:'2-digit', minute:'2-digit'})}</Typography>
                                    </Box>
                                    <Typography variant="caption" color={themeColors.textSecondary} sx={{fontSize:'0.65rem'}}>Guardia: {log.guardia_nombre || 'Sistema'}</Typography>
                                </Box>
                            ))}
                        </Paper>
                    </Box>
        ...
        {/* COLUMNA 3: CHAT (Igual que antes) */}
                ...
                <Box sx={{ p: 2, borderBottom: `1px solid ${themeColors.bgLight}`, bgcolor: chatActivo ? themeColors.chatGuardia + '20' : 'transparent', display:'flex', alignItems:'center', justifyContent: 'space-between' }}>
                    {chatActivo ? (
                        <Box display="flex" alignItems="center">
                            ...
                            <Box overflow="hidden"><Typography variant="subtitle2" color={themeColors.textPrimary} noWrap fontWeight="bold">{chatActivo.first_name}</Typography></Box>
                        </Box>
                    ) : <Typography variant="subtitle1" fontWeight="bold" color={themeColors.textPrimary}>Comunicación</Typography>}
                    {chatActivo && <Tooltip title={verArchivados ? "Ver Mensajes Activos" : "Ver Archivados"}><IconButton onClick={() => setVerArchivados(!verArchivados)} sx={{color: verArchivados ? themeColors.warning : themeColors.textSecondary}}><ArchiveIcon /></IconButton></Tooltip>}
                </Box>
                <Box sx={{ flexGrow: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
                    {!chatActivo ? (
                        <>
                            <Box sx={{ p: 1.5, borderBottom: `1px solid ${themeColors.bgLight}` }}><CustomTextField size="small" placeholder="Buscar vecino..." value={busqueda} onChange={(e) => setBusqueda(e.target.value)} InputProps={{ startAdornment: <InputAdornment position="start"><SearchIcon sx={{color:themeColors.textSecondary}}/></InputAdornment>, sx: { borderRadius: 4, bgcolor: themeColors.bgLight } }} /></Box>
                            <List sx={{ overflowY: 'auto', flexGrow: 1 }}>{vecinosFiltrados.map(v => (<ListItem button key={v.id} onClick={() => seleccionarVecino(v)} sx={{ '&:hover': { bgcolor: themeColors.bgLight }, borderBottom: `1px solid ${themeColors.bgLight}40` }}><ListItemAvatar><Avatar sx={{ bgcolor: themeColors.bgLight, color: themeColors.accent, fontWeight:'bold' }}>{v.username.charAt(0).toUpperCase()}</Avatar></ListItemAvatar><ListItemText primary={<Typography color={themeColors.textPrimary} fontWeight={500}>{v.first_name} {v.last_name}</Typography>} secondary={<Typography color={themeColors.textSecondary} variant="caption">Casa {v.casa_id || '?'}</Typography>} /></ListItem>))}</List>
                        </>
                    ) : (
                        <>
                                ...
                                {mensajes.map((msg, i) => (
                                    <Box key={i} sx={{ alignSelf: msg.es_mio ? 'flex-end' : 'flex-start', maxWidth: '85%', position: 'relative', '&:hover .archive-btn': {opacity: 1} }}>
                                        <Paper elevation={1} sx={{ p: 1.5, bgcolor: msg.es_mio ? themeColors.chatGuardia : themeColors.chatVecino, color: '#f1f5f9', borderRadius: '12px', borderBottomRightRadius: msg.es_mio?0:12, borderBottomLeftRadius:!msg.es_mio?0:12, pr: 4 }}>
                                            ...
                                            <Tooltip title={verArchivados ? "Restaurar" : "Archivar"}><IconButton className="archive-btn" size="small" onClick={() => handleToggleArchivo(msg.id, !verArchivados)} sx={{ position: 'absolute', top: -10, right: -10, opacity: 0, transition: '0.2s', bgcolor: themeColors.bgPaper, border: `1px solid ${themeColors.bgLight}`, color: verArchivados ? themeColors.warning : themeColors.success, '&:hover':{bgcolor:themeColors.bgLight} }}>{verArchivados ? <RestoreFromTrashIcon fontSize="small"/> : <CheckCircleIcon fontSize="small" />}</IconButton></Tooltip>
                                        </Paper>
                                    </Box>
                                ))}
                                <div ref={chatBottomRef} />
                            </Box>
                            {!verArchivados && (<Box sx={{ p: 1.5, bgcolor: themeColors.bgPaper, borderTop: `1px solid ${themeColors.bgLight}`, display: 'flex', alignItems:'center' }}><CustomTextField fullWidth size="small" placeholder="Responder..." value={nuevoMensaje} onChange={(e) => setNuevoMensaje(e.target.value)} onKeyPress={(e) => e.key === 'Enter' && enviarMensaje()} sx={{ mr: 1, '& .MuiOutlinedInput-root': { borderRadius: 4, bgcolor: themeColors.bgLight } }} /><IconButton sx={{ bgcolor: themeColors.accent, color: 'white', '&:hover': { bgcolor: themeColors.accentHover }, width:40, height:40 }} onClick={enviarMensaje} disabled={!nuevoMensaje.trim()}><SendIcon fontSize="small" /></IconButton></Box>)}
                        </>
                    )}
                </Box>
            </Paper>
      ...
      <Dialog open={openHistorial} onClose={()=>setOpenHistorial(false)} fullWidth maxWidth="sm">
          <DialogTitle sx={{bgcolor: themeColors.bgPaper, color: 'white'}}>Consultar Bitácora Histórica</DialogTitle>
          <DialogContent sx={{bgcolor: themeColors.bgMain, color: 'white'}}>
              <Box py={2}>
                  <CustomTextField type="date" label="Seleccionar Fecha" InputLabelProps={{shrink: true}} value={fechaHistorial} onChange={(e)=>setFechaHistorial(e.target.value)} />
                  <Button variant="contained" fullWidth sx={{mt:2}} onClick={buscarHistorial}>BUSCAR</Button>
              </Box>
              <Box mt={2} sx={{maxHeight: 300, overflowY: 'auto'}}>
                  {listaHistorial.map((h, i) => (
                      <Box key={i} p={1} mb={1} sx={{borderBottom: '1px solid gray'}}>
                          <Typography variant="body2">{h.mensaje}</Typography>
                          <Typography variant="caption" color="gray">{new Date(h.fecha).toLocaleString()}</Typography>
                      </Box>
                  ))}
              </Box>
          </DialogContent>
