...
function Dashboard() {
  ...
  const [chatDestinatario, setChatDestinatario] = useState(null); 
  ...
  useEffect(() => {
      if (openCaseta || isGuard) {
          cargarMensajes();
          const t = setInterval(cargarMensajes, 3000); 
  ...
  const handleResponder = (msg) => {
      setChatDestinatario({ 
          id: msg.remitente, 
          nombre: msg.remitente_nombre || 'Vecino' 
      });
  };
  ...
  const handleGuardarReporteDiario = async () => { 
      ...
      try{
          ...
          setNuevoReporte(''); 
          enqueueSnackbar("Evento registrado en bitácora",{variant:'success'}); 
          cargarDatosVigilancia(); 
        ...
        <Box sx={{ flexGrow: 1, height: '100vh', display: 'flex', flexDirection: 'column', bgcolor: darkTheme.bg, color: darkTheme.text, overflow: 'hidden' }}>
                    ...
                    <Grid size={{ xs: 12, md: 3 }} sx={{ height: '100%', display: 'flex', flexDirection: 'column', gap: 2 }}>
                        ...
                        <Paper sx={{ flexGrow: 1, bgcolor: '#1e293b', borderRadius: 2, border: `1px solid ${darkTheme.border}`, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
                            <Box sx={{ p: 1, bgcolor: '#0f172a', borderBottom: `1px solid ${darkTheme.border}`, display:'flex', justifyContent:'space-between', alignItems:'center' }}>
                                ...
                                {chatDestinatario && (
                                    <Chip label={`@${chatDestinatario.nombre}`} size="small" color="primary" onDelete={()=>setChatDestinatario(null)} deleteIcon={<CancelIcon style={{color:'white'}}/>} sx={{maxWidth: 150}} />
                                )}
                            </Box>
                                ...
                                {chatMessages.map((msg) => (
                                    <Box key={msg.id} sx={{ mb: 1, p: 1, borderRadius: 1, bgcolor: msg.es_para_guardia ? '#334155' : '#0ea5e9', alignSelf: msg.es_para_guardia ? 'flex-start' : 'flex-end', maxWidth:'90%' }}>
                                        <Box display="flex" justifyContent="space-between" alignItems="center">
                                            ...
                                            {msg.es_para_guardia && (
                                                <IconButton size="small" onClick={()=>handleResponder(msg)} sx={{color:'#94a3b8', p:0, ml:1}} title="Responder"><ReplyIcon fontSize="inherit"/></IconButton>
                                            )}
                                        </Box>
                            ...
                            <Box sx={{ p: 1, bgcolor: '#0f172a', display:'flex', gap:1 }}>
                                <TextField fullWidth size="small" placeholder={chatDestinatario ? "Responder..." : "Mensaje..."} value={nuevoMensaje} onChange={(e)=>setNuevoMensaje(e.target.value)} sx={{bgcolor:'#1e293b', borderRadius:1, input:{color:'white'}}} onKeyPress={(e)=>e.key==='Enter' && enviarMensaje()} />
                        ...
                        <Card 
                            ...
                            sx={{ 
                                bgcolor: '#f57c00', color: 'white', cursor: 'pointer', borderRadius: 2, 
                                textAlign: 'center', py: 2, display:'flex', alignItems:'center', justifyContent:'center', gap: 2, 
                                transition: '0.3s', '&:hover': { bgcolor: '#ef6c00' } 
                            }}
                        >
                            <EditNoteIcon sx={{ fontSize: 40 }} />
                            <Box textAlign="left">
                                <Typography variant="h6" fontWeight="bold" lineHeight={1.1}>BITÁCORA</Typography>
                                <Typography variant="caption" sx={{opacity:0.8}}>Registrar Novedades</Typography>
                            </Box>
                        </Card>
            ...
            <Dialog open={openBitacora} onClose={() => setOpenBitacora(false)} fullWidth maxWidth="md" PaperProps={{sx:{bgcolor: darkTheme.bg, color:'white', border: `1px solid ${darkTheme.border}`}}}>
                    ...
                    <Grid container sx={{height: '500px'}}>
                        <Grid size={{ xs: 12, md: 7 }} sx={{borderRight: `1px solid ${darkTheme.border}`, overflowY:'auto', p:2}}>
                            ...
                            {reportesDiarios.map(r => (
                                <Box key={r.id} sx={{ mb: 2, display: 'flex', gap: 1.5 }}>
                                    <Box sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center', minWidth: '50px' }}>
                                        <Typography variant="caption" sx={{ color: '#38bdf8', fontWeight: 'bold', fontFamily: 'monospace' }}>
                                            {new Date(r.fecha).toLocaleTimeString([], {hour:'2-digit', minute:'2-digit'})}
                                        </Typography>
                                        <Box sx={{ width: '2px', flexGrow: 1, bgcolor: '#334155', mt: 0.5, mb: -1.5 }} />
                                    </Box>
                                    <Box sx={{ bgcolor: '#1e293b', p: 1.5, borderRadius: 2, width: '100%', border: `1px solid ${darkTheme.border}` }}>
                                        <Typography variant="caption" color="#94a3b8" display="block">
                                            {r.guardia_nombre || 'Guardia'}
                                        </Typography>
                                        <Typography variant="body2" sx={{color:'white'}}>{r.mensaje}</Typography>
                                    </Box>
                                </Box>
                            ))}
                        </Grid>
                        ...
                        <Grid size={{ xs: 12, md: 5 }} sx={{bgcolor: '#1e293b', p: 3, display:'flex', flexDirection:'column'}}>
                            ...
                            <TextField 
                                ...
                                rows={10} 
                                variant="filled" 
                                placeholder="Describa el suceso detalladamente..." 
                                value={nuevoReporte} 
                                onChange={(e)=>setNuevoReporte(e.target.value)} 
                                sx={{ bgcolor: '#0f172a', borderRadius: 1, flexGrow:1, textarea: { color: 'white' } }} 
                            />
                            <Button 
                                variant="contained" 
                                size="large" 
                                startIcon={<SaveIcon/>} 
                                onClick={handleGuardarReporteDiario} 
                ...
                <DialogActions sx={{bgcolor: '#0f172a'}}>
                    <Button onClick={() => setOpenBitacora(false)} sx={{color:'white'}}>Cerrar Bitácora</Button>
                </DialogActions>
            </Dialog>
  ...
  return (
    <Box sx={{ flexGrow: 1, minHeight: '100vh', display: 'flex', flexDirection: 'column', bgcolor: '#f5f5f5' }}>
      ...
      <Dialog open={openCaseta} onClose={()=>setOpenCaseta(false)} fullWidth maxWidth="sm">
          ...
          <DialogContent sx={{pt:0, height: 450, display:'flex', flexDirection:'column'}}>
